...
**官方资源：**
- GitHub 仓库: https://github.com/anomalyco/opencode [^1]
- DeepWiki 文档: https://deepwiki.com/anomalyco/opencode
...
OpenCode 内置了预定义的 Agent 类型系统，每个 Agent 都有明确的模式分类，核心类型定义位于 `packages/opencode/src/agent/`:
...
**[anomalyco/opencode → packages/opencode/src/agent/types.ts]**
...
Agent 通过配置驱动方式定义，位于 `packages/opencode/src/agent/`:
1. 静态默认定义在代码中 **[anomalyco/opencode → packages/opencode/src/agent]**
2. 用户配置文件中的定义覆盖/扩展默认配置
3. 支持自定义 Agent，可指定权限、模型、温度等参数

### 反应循环架构

OpenCode 使用 **ACP (Agent Client Protocol)** 处理会话，通过 `SessionProcessor` 处理每个 Agent 回合，核心位于 `packages/opencode/src/session/`:

- 基于 Bus 事件系统，反应式更新 **[anomalyco/opencode → packages/opencode/src/session]**
- 每个消息作为包含多个 Part 的结构处理
- Part 可以是 text、tool、file、compaction 等类型
- 处理过程异步可中止，支持用户取消

Zed extension 支持 ACP 协议实现位于 `sdks/zed/` **[anomalyco/opencode → sdks/zed/]**

**参考来源：**
- DeepWiki Session processing: https://deepwiki.com/anomalyco/opencode/packages/opencode/src/session [^2]

---

## 2. 上下文管理与上下文压缩

OpenCode 实现了**两级上下文压缩**: **pruning 修剪** + **LLM 摘要压缩**，核心实现位于:
- Pruning and compaction orchestration: `packages/opencode/src/session/` **[anomalyco/opencode → packages/opencode/src/session]**
- LLM compaction: `packages/opencode/src/session/compaction.ts`, `packages/opencode/src/session/summary.ts` **[anomalyco/opencode]**
...
**参考来源：**
- DeepWiki Context Compaction: https://deepwiki.com/anomalyco/opencode/packages/opencode/src/session [^2]

---

## 3. SubAgent 创建

Parallel agent execution and subtask management is handled by the `task` tool, located in `packages/opencode/src/tool/task.ts` **[anomalyco/opencode → packages/opencode/src/tool/task.ts]** that provides parallel agent execution / subtask management for complex operations.

### 声明式 SubAgent 定义

SubAgent 在配置中声明定义，每个 subagent 有，核心类型位于 `packages/opencode/src/agent/`:
...
**参考来源：**
- DeepWiki Task tool: https://deepwiki.com/anomalyco/opencode/packages/opencode/src/tool [^2]

---

## 4. Skill 机制

OpenCode 兼容标准 Agent Skills 格式，同时支持远程 Skill 仓库拉取， documented in section 2.11 *Skills & Command System* on DeepWiki **[anomalyco/opencode]**.
...
**参考来源：**
- DeepWiki Skills system: https://deepwiki.com/anomalyco/opencode [^2]

---

## 5. Agent 会话隔离

OpenCode 使用**关系数据库持久化 + 父子引用**实现会话隔离和分叉， session management core located in `packages/opencode/src/session/` **[anomalyco/opencode → packages/opencode/src/session]**. Enterprise session sharing is in `packages/enterprise/` with multiple concurrent sessions and shareable links stored by default locally **[anomalyco/opencode → packages/enterprise/]**.
...
创建子会话：
```typescript
// From: packages/opencode/src/session/types.ts
...
**参考来源：**
- DeepWiki Session management: https://deepwiki.com/anomalyco/opencode/packages/opencode/src/session [^2]
...
[^3]: CSDN - GitHub 热榜项目 日榜精选(2026-01-12) - https://blog.csdn.net/2401_87766320/article/details/156839614
[^4]: CSDN - 【笔记】Windows 上安装 OpenCode AI 编码助理 - https://blog.csdn.net/u014451778/article/details/156748504
[^5]: CSDN - OpenCode:开源 AI Coding Agent 技术与行业分析 - https://blog.csdn.net/fyfugoyfa/article/details/157844145
[^6]: CSDN - OpenCode 深度解析:2025 年最强开源 AI 编程助手完全指南 - https://blog.csdn.net/Yunyi_Chi/article/details/157775001
[^7]: CSDN - AI 编程助手三强争霸:OpenCode vs Claude Code vs Kimi Code CLI 深度对比 - https://blog.csdn.net/qq_41797451/article/details/158039912
